<nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
    <div class="container">
        <a class="navbar-brand" href="{{ route('page.home') }}">
            <div class="brand-icon"><i class="fa fa-plane"></i></div>
            <div class="brand-texts">
                <span class="brand-main">Travel</span>
                <span class="brand-sub">Du lịch cùng chúng tôi</span>
            </div>
        </a>
                ...
                @if (Auth::guard('users')->check())
                    ...
                    <li class="nav-item {{ request()->is('thong-tin-tai-khoan.html') || request()->is('thay-doi-mat-khau.html') || request()->is('danh-sach-tour.html') ? 'active' : '' }}">
                        <a href="{{ route('info.account') }}" class="nav-link" title="{{ $user->name }}">
                            <i class="fa fa-user-circle mr-1"></i> {{ the_excerpt($user->name, 15) }}
                        </a>
                    </li>
                    <li class="nav-item"><a href="{{ route('page.user.logout') }}" class="nav-link">Đăng xuất</a></li>
                @else
                    <li class="nav-item {{ request()->is('dang-ky-tai-khoan.html') ? 'active' : '' }}"><a href="{{ route('user.register') }}" class="nav-link">Đăng ký</a></li>
                    <li class="nav-item cta {{ request()->is('dang-nhap.html') ? 'active' : '' }}">
                        <a href="{{ route('page.user.account') }}" class="nav-link">
